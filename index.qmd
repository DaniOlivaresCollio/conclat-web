---
title: "Laboratorio de Investigación Social Abierta"
pagetitle: "LISA Website"
page-layout: custom
format: html
page-navigation: false
toc: false
about:
  image: "resources/imagenes/sharing-default.png"
  id: hero-heading
  template: jolla
  image-shape: rectangle
  
listing:
  - id: componentes
    contents: "componentes/*/index.qmd"
    sort: "date desc"
    template: resources/custom-listings/home-components.ejs
    
  - id: noticias
    contents: 
    - noticias/posts/*/*.qmd
    - "!noticias/posts/drafts/*qmd"
    sort: date desc
    type: grid
    grid-columns: 3
    image-height: 150px
    fields: [image, title, description]
    max-items: 6
    categories: false
    sort-ui: false
    filter-ui: false
    feed: false
    
  - id: actividades
    contents: 
    - actividades/presentaciones/*/*.qmd
    - "!actividades/posts/drafts/*qmd"
    sort: date desc
    type: default
    image-height: 150px
    fields: [title, date, author, description]
    max-items: 3
    categories: false
    sort-ui: false
    date-format: "MMMM YYYY"
    feed: false
    page-size: 100
    
  - id: recomendados
    contents: 
    - recomendados/posts/*/*.qmd
    - "!recomendados/posts/drafts/*qmd"
    sort: date desc
    type: grid
    grid-columns: 3
    image-height: 150px
    fields: [image, title, description]
    max-items: 6
    categories: false
    sort-ui: false
    filter-ui: false
    feed: false
    
  - id: equipo
    template: resources/custom-listings/people-box.ejs
    contents: equipo/posts/*/*.qmd
    sort: date desc
    type: grid
---

:::hero-heading
![](resources/imagenes/sharing-default.png){.img-fluid .mb-3 width=90% fig-align="center"}
:::

::: {.parallax-container}
::: {.parallax-image-container2}
::: {.button style="text-align: center; padding-top: 7em;"}
<p align="center" style="font-size:48px; font-weight:bold; color:white;">
<i class="bi bi-newspaper" style="margin-right: 10px;"></i>Sobre LISA</p>


<p style="font-size:36px; color:white; text-align:center;">
El Laboratorio de Investigación Social Abierta es una iniciativa del Centro de Estudios de Conflicto y Cohesión Social (COES) que tiene como propósito ser una guía para el desarrollo de una ciencia social abierta y reproducible para la comunidad científica y la ciudadanía.
</p>

:::
:::
:::

::: {.section-text}
::: {.section-container}
::: {.button style="text-align: center; padding-top: 7em;"}

<p align="center" style="font-size:36px; font-weight:bold;">
<i class="bi bi-puzzle-fill" style="margin-right: 10px;"></i>Componentes</p>




::: {#componentes}
:::


:::
:::
:::


::: {.parallax-container}
::: {.parallax-image-container}
::: {.button style="text-align: center; padding-top: 7em;"}
<p align="center" style="font-size:36px; font-weight:bold; color:white;">
<i class="bi bi-newspaper" style="margin-right: 10px;"></i>Noticias</p>



::: {#noticias}
:::


:::
:::
:::

::: {.section-text}
::: {.section-container}
::: {.section-block}
::: {style="text-align: left;"}

<p align="center" style="font-size:36px; font-weight:bold;">
<i class="bi bi-calendar-week" style="margin-right: 10px;"></i>Actividades</p>

:::{#actividades}
:::

::: {.button}


[Todos los eventos](actividades/actividades.qmd){.btn-action-primary .btn-action .btn .btn-success .btn-lg role="button" style="background-color: #0693e3; margin-top: 0em;"}
:::

:::
:::
:::
:::

::: {.parallax-container}
::: {.parallax-image-container}
::: {.button style="text-align: center; padding-top: 7em;"}
<p align="center" style="font-size:36px; font-weight:bold; color:white;">
<i class="bi bi-journals" style="margin-right: 10px;"></i>Recomendados</p>

::: {#recomendados}
:::


:::
:::
:::




::: {.section-text}
::: {.section-container}
::: {.section-block}
::: {style="text-align: center;"}

<p align="center" style="font-size:36px; font-weight:bold;">
<i class="bi bi-people-fill" style="margin-right: 10px;"></i>Equipo</p>

::: {#equipo}
:::

::: {.button style="text-align: right;"}
[Pasaron por LISA>>](/alumni/index.qmd){.btn-template-primary .btn-action .btn .btn-success .btn-lg role="button" style="margin-top: 0em;"}
:::

:::
:::
:::
:::

::: {.section-text .logos-bg}
::: {.section-container}
::: {.section-block}
::: {style="text-align: center;"}


::: {layout-ncol=5}

[![](resources/imagenes/anid.png)](https://anid.cl)

[![](resources/imagenes/uch.png)](https://uchile.cl)

[![](resources/imagenes/puc.png)](https://www.uc.cl)

[![](resources/imagenes/udp.png)](https://www.udp.cl)

[![](resources/imagenes/uai.png)](https://www.uai.cl)

:::




:::
:::
:::
:::
